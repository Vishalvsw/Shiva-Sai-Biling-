            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md space-y-4">
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-[#143A78] rounded-lg hover:bg-blue-900">Save</button>
                    </div>
                </form>
            </div>
        )
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg space-y-4">
                <div className="flex justify-end">
                    <button onClick={() => setIsCreating(true)} className="px-4 py-2 text-sm font-medium text-white bg-[#143A78] rounded-lg hover:bg-blue-900">
                        Add New User
                    </button>
                </div>
                            ...
                            {Object.entries(users).map(([username, data]: [string, AppUser]) => (
                                <tr key={username}>
                                    ...
                                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 space-x-2">
                                        <button onClick={() => setEditingUser({ username, data })} className="text-orange-600 hover:text-orange-900">Edit</button>
