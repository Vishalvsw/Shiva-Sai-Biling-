            ...
            <div className="flow-root print:mt-4">
                <table className="min-w-full divide-y divide-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                        ...
                        {items.map((item) => (
                            <tr key={item.id}>
                                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-slate-900 sm:pl-6">{item.name}</td>
                                <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-500 text-right">₹{item.price.toFixed(2)}</td>
                                <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-500 text-right print:hidden">
                                 ...
                                 <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-500 text-right">₹{(item.price - item.discount).toFixed(2)}</td>
            ...
            <div className="mt-6 border-t border-slate-200 pt-6 space-y-2 text-right print:mt-8 print:pt-4 print:border-t-2 print:border-slate-800">
                ...
                {totalDiscountAmount > 0 && (
                    <div className="flex justify-end gap-4">
                        ...
                        <span className="text-sm text-red-600 w-28">- ₹{totalDiscountAmount.toFixed(2)}</span>
                    </div>
                )}
                
                <div className="flex justify-end gap-4">
                    <span className="text-sm font-medium text-slate-500">Taxable Amount:</span>
                    <span className="text-sm text-slate-900 w-28">₹{taxableAmount.toFixed(2)}</span>
                </div>
